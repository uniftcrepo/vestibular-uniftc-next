...
const ButtoesFdi = ({ match, location }) => {
  ...
  const [atualizacaoFdi, setAtualizacaoFdi] = useState(DATA.fdi);
  const [token, setToken] = useState();

  useEffect(() => {
    const query = new URLSearchParams(location.search);
    const tokenUrlRedirecionamento = query.get("token");

    if (tokenUrlRedirecionamento == "true/") {
      setToken("sixth");
      console.log(tokenUrlRedirecionamento);
    } else {
      setToken("first");
      console.log(tokenUrlRedirecionamento);
    }
  ...
  if (hash.utm_source !== undefined) {
    ...
    utm =
      "?utm_source=lp_ps&utm_medium=acesso_direto&utm_campaign=vestibular_2024.2&utm_content=inscricao";
  }
    ...
    <Container id="forma_de_ingresso" className="container">
      ...
      <DivBotoes>
        ...
        >
          {" "}
          {DATA &&
            DATA.abasinscricao.map((x) => {
              return (
                <Tab key={x.posicao}
                  eventKey={x.posicao}
                  title={parse(x.nome_no_acompanhamento)}
                ...
                >
                  ...
                  <ContainerCursosSeparacao>
                    <Col lg={7}>
                      ...
                      <div className="texto_legal">{parse(x.texto_legal)}</div>
                      <br />
                    ...
                    <Col lg={5}>
                      <Botoes
                        ...
                        prazo4={botao_de_inscricao_ate(
                          x.botao_de_inscricao_ate4
                        )}
                      />
                      {habilitarform && habilitarbutaoinscricao && (
                        <Formulario {...match} concurso={x.concurso} />
                      )}
                    </Col>
                  </ContainerCursosSeparacao>
                  <br />
